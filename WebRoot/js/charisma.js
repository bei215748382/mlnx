$(document)
		.ready(
				function() {
					...
					$('#themes a[data-value="' + current_theme + '"]')
					...
					// other things to do on document ready, seperated for ajax
					// calls
					docReady();
				});
...
function docReady() {
	...
	$('.sortable').sortable({
		...
		update : function(event, ui) {
			// line below gives the ids of elements, you can make ajax call here
	...
	if ($('.tour').length && typeof (tour) == 'undefined') {
		...
		tour.addStep({
			element : ".span10:first", /*
										 * html element next to which the step
